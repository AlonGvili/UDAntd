function New-UDAntdStepsItem {
    [CmdletBinding()]
    [OutputType('UDAntd.StepsItem')]
    param(
    ...
    End {
        ...
        if ($null -ne $Content) {
            if ($IsEndpoint) {
                if ($Content -is [scriptblock]) {
                    $StepsItemContentEndpoint = New-UDEndpoint -Endpoint $Content -Id $Id
                    $StepsItemContent = $Content.Invoke()
                }
        ...
        $UDAntdStepsItem.PSTypeNames.Insert(0, 'UDAntd.StepsItem')
        $UDAntdStepsItem
    }
}
