...
/******/ 		return __webpack_require__.p + "" + ({"1":"ud-chart","2":"ud-monitor","3":"ud-link","4":"ud-input","5":"ud-counter","6":"ud-html","7":"ud-grid","8":"ud-date-time","9":"ud-element","10":"ud-icon","11":"ud-loading","12":"ud-page-cycler","13":"ud-modal","15":"vendors~ud-date-time","16":"vendors~ud-icon"}[chunkId]||chunkId) + "." + "1b65414f142ebbca7676" + ".bundle.js"
/******/ 	}
/******/
...
function ud_dashboard_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
...
function (_React$Component) {
  ...
  ud_dashboard_createClass(UdDashboard, [{
    ...
    value: function connectWebSocket(sessionId) {
      ...
      connection.on('setLocalStorageItem', function (_ref) {
        var key = _ref.key,
            content = _ref.content;
        console.log("".concat(key), content);
        window.localStorage.setItem(key, JSON.stringify(_toConsumableArray(content)));

        var value = ud_dashboard_defineProperty({}, key, JSON.parse(window.localStorage.getItem(key)));

        value[key] ? console.log("[ SetLocalStorageItem : ".concat(key, " ][ Passed ] Content is not empty")) : console.log("[ SetLocalStorageItem : ".concat(key, " ][ Error ] Content is empty"));
      });
      connection.on('getLocalStorageItem', function (requestId, key) {
        var value = ud_dashboard_defineProperty({}, key, JSON.parse(window.localStorage.getItem(key)));

        value[key] ? UniversalDashboard.post("/api/internal/component/element/sessionState/localStorage/".concat(requestId), value) : console.log("[ GetLocalStorageItem : ".concat(key, " ][ Error ] Content is empty"));
      });
      connection.on('removeLocalStorageItem', function (key) {
        window.localStorage.removeItem(key);
        var item = window.localStorage.getItem(key);
        return item ? console.log("problem removing ".concat(key, " from local storage")) : console.log("".concat(key, " was remove from localStorage"));
      });
...
/******/ });
//# sourceMappingURL=main.1b65414f142ebbca7676.bundle.map