...
/******/ 		return __webpack_require__.p + "" + ({"1":"ud-chart","2":"ud-monitor","3":"ud-link","4":"ud-input","5":"ud-counter","6":"ud-html","7":"ud-grid","8":"ud-date-time","9":"ud-element","10":"ud-icon","11":"ud-loading","12":"ud-page-cycler","13":"ud-modal","15":"vendors~ud-date-time","16":"vendors~ud-icon"}[chunkId]||chunkId) + "." + "c60f2ed24a852b8aa6b1" + ".bundle.js"
/******/ 	}
/******/
...
// CONCATENATED MODULE: ./src/app/services/hooks/useDashboardEvent.jsx
...
function useDashboardEvent_objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i] != null ? arguments[i] : {}; var ownKeys = Object.keys(source); if (typeof Object.getOwnPropertySymbols === 'function') { ownKeys = ownKeys.concat(Object.getOwnPropertySymbols(source).filter(function (sym) { return Object.getOwnPropertyDescriptor(source, sym).enumerable; })); } ownKeys.forEach(function (key) { useDashboardEvent_defineProperty(target, key, source[key]); }); } return target; }

function useDashboardEvent_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
...
var REMOVE_ELEMENT = "removeElement";
var ADD_ELEMENT = "addElement";
var CLEAR_ELEMENT = "clearElement";
var SYNC_ELEMENT = "syncElement"; // const SET_LOCALSTORAGE = "setLSItem";
// const GET_LOCALSTORAGE = "getLSItem";
// const REMOVE_LOCALSTORAGE = "removeLSItem";
// const CLEAR_LOCALSTORAGE = "clearLS";

function useDashboardEvent(elementId, initialState) {
  var content = initialState.content,
      attributes = _objectWithoutProperties(initialState, ["content"]);
  ...
  Object(react["useEffect"])(function () {
    ...
    var pubSubToken = UniversalDashboard.subscribe(elementId, events);
    return function () {
      return UniversalDashboard.unsubscribe(pubSubToken);
    };
  }, [elementId, initialState]);
  var events = Object(react["useCallback"])(function (msg, event) {
    switch (event.type) {
      // Set-UDElement
      case SET_STATE:
        setState(function (state) {
          return {
            attributes: useDashboardEvent_objectSpread({}, state.attributes, event.state.attributes),
      ...
      case REQUEST_STATE:
        UniversalDashboard.post("/api/internal/component/element/sessionState/".concat(event.requestId), useDashboardEvent_objectSpread({}, state));
        console.log('invoke request state from useDashboardEvent with the state => ', state);
        break;
      // Add-UDElement

      case ADD_ELEMENT:
        setState(function (state) {
          return useDashboardEvent_objectSpread({}, state, {
      ...
      case REMOVE_ELEMENT:
        setState(function (state) {
          var newStateContent = state.content;
          newStateContent.splice(-1, 1);
          return useDashboardEvent_objectSpread({}, state, {
            content: _toConsumableArray(newStateContent)
          });
        });
        break;
      // Clear-UDElement
      ...
      //     // window.localStorage.setItem(event.itemKey, JSON.stringify(event.itemValue))
      //     // const item = window.localStorage.getItem(event.itemKey)
      //     // item ? console.log('item was added to local storage') : console.log('problem adding item to local storage')
      //     break;
      // Get-LocalStorage
      // case GET_LOCALSTORAGE:
      //     const item = window.localStorage.getItem(event.itemKey)
      //     item ? console.log('Get-LocalStorage: ', event.itemKey) : console.log(`problem getting ${event.itemKey} from local storage`)
      //     setState(state => {
      //         return {
      //             ...state,
      //             attributes: {
      ...
      //     window.localStorage.removeItem(event.itemKey)
      //     const item = window.localStorage.getItem(event.itemKey)
      //     item ?  console.log(`problem removing ${event.itemKey} from local storage`) : console.log(`${event.itemKey} was remove from localStorage`)
      //     break;
      // // Clear-LocalStorage
      // case CLEAR_LOCALSTORAGE:
      //     window.localStorage.clear()
      //     const items = window.localStorage.length
      //     items > 0 ?  console.log(`problem clearing  local storage`) : console.log(`localStorage was cleared!`)
      //     break;
      // Just break
  ...
  var reload = Object(react["useCallback"])(function () {
    UniversalDashboard.get("/api/internal/component/element/".concat(elementId), function (data) {
...
// CONCATENATED MODULE: ./src/app/ud-localstorage.jsx
function ud_localstorage_slicedToArray(arr, i) { return ud_localstorage_arrayWithHoles(arr) || ud_localstorage_iterableToArrayLimit(arr, i) || ud_localstorage_nonIterableRest(); }

function ud_localstorage_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance"); }

function ud_localstorage_iterableToArrayLimit(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"] != null) _i["return"](); } finally { if (_d) throw _e; } } return _arr; }

function ud_localstorage_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }




var ud_localstorage_getLocalStorageItem = function getLocalStorageItem(props) {
  var _useDashboardEvent = useDashboardEvent(props.id, props),
      _useDashboardEvent2 = ud_localstorage_slicedToArray(_useDashboardEvent, 3),
      state = _useDashboardEvent2[0],
      reload = _useDashboardEvent2[1],
      setState = _useDashboardEvent2[2];

  var content = state.content,
      attributes = state.attributes;
  var item = JSON.parse(window.localStorage.getItem(attributes.itemKey));
  console.log("".concat(attributes.itemKey, " => "), item);
  return item;
};

/* harmony default export */ var ud_localstorage = (ud_localstorage_getLocalStorageItem);
  ...
  ud_dashboard_createClass(UdDashboard, [{
    ...
    value: function connectWebSocket(sessionId) {
      ...
      connection.on('getLocalStorageItem', function (itemKey) {
        var itemValue = ud_localstorage(itemKey);
        console.log('[connection.on] => getLocalStorageItem => ', itemValue); // PubSub.publish(componentId, {
        //     type: "requestState",
        //     requestId: requestId
        // });
      });
...
/******/ });
//# sourceMappingURL=main.c60f2ed24a852b8aa6b1.bundle.map